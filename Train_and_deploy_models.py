...
with mlflow.start_run(run_name="amazon-category-model") as run:
    ...
    model_version = mlflow.register_model(
        model_uri=f"runs:/{mlflow_run_id}/model", name="amazon-category-model"
    )
...
response = requests.post(
    ...
    json={
        "inputs": [
            "Oh man, the best cereals I've ever had! A lot of nuts, and I love nuts.",
...
from amazon_reviews.recommender_model.amazon_product_recommender import (
    AmazonProductRecommenderWrapper,
...
with mlflow.start_run(run_name="amazon-recommender") as run:
    ...
    conda_env = _mlflow_conda_env(
        ...
        additional_pip_deps=[
            "code/amazon_reviews-0.0.1-py3-none-any.whl",
            "pyspark==3.3.0",
        ],
    ...
    model_version = mlflow.register_model(
        model_uri=f"runs:/{mlflow_run_id}/model", name="amazon-recommender"
    )
