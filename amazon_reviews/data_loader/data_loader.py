...
class AmazonReviewsDataLoader:
        ...
        for cat in self.list_to_download:
            if not os.path.exists(f"{os.path.join(self.path, cat)}.txt"):
                data = urllib.request.urlopen(
                    f"https://archive.org/download/amazon-reviews-1995-2013/{cat}.txt.gz"
                ).read()
    ...
    def create_reviews_dataframe(self):
        ...
        with codecs.open(
            os.path.join(self.path, "categories.txt"), "r", "latin_1"
        ) as f:
            categories = f.read()
        ...
        for col in ["Cat1", "Cat2", "Cat3"]:
            df[col] = df[col].apply(
                lambda x: str.lower(re.sub("[^a-zA-Z]", " ", x))
                .replace("  ", " ")
                .strip()
            )
        df = df[
            df.Cat1.isin(
                ["pet supplies", "baby products", "beauty", "grocery  gourmet food"]
            )
        ]
